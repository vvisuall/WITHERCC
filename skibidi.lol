...
Menu.Notify("Skibidi.lol has been injected", 5)
Menu.Tab("Aimbot")
Menu.Tab("Visuals")
Menu.Tab("Settings")

Menu:SetTitle("Skibidi.lol | Lite Free Build")
...
Menu.Hotkey("Aimbot", "Aim Assist", "Aimbot Keybind", false, function(x) -- making a togglekey, again replace Main and Target Aimbot with your tab and section name.
    Skibidi.Keybind = x
end)

Menu.CheckBox("Aimbot", "Aim Assist", "Resolver", false, function(x) -- creating a toggle, replace "Main" with your tab name, and replace "Target Aimbot" with ur section name.
    Skibidi.Resolver = x
end)

Menu.TextBox("Aimbot", "Aim Assist", "Resolver Interval", "0.1", function(x)
    Interval = x
end)

Menu.ComboBox("Aimbot", "Aim Assist", "Resolver Version", "Recalculate", {"Recalculate", "MoveDirection"}, function(x)
    ResolverVersion = x                                                                                                           
end)


Menu.TextBox("Aimbot", "Aim Assist", "Prediction", "0.13772123", function(x)
    Skibidi.Prediction = x
end)




Menu.Button("Aimbot", "Aim Assist", "Bypass Camlock Anti Cheat", function(x)
...
Menu.Hotkey("Settings", "Config", "Menu Keybind", false, function(x) -- making a togglekey, again replace Main and Target Aimbot with your tab and section name.
    Skibidi.MenuKeybind = x
end)


Menu.CheckBox("Visuals", "Esp", "Box ESP", false, function(x) -- creating a toggle, replace "Main" with your tab name, and replace "Target Aimbot" with ur section name.
    Skibidi.BoxEsp = x
end)
...
UserInputService.InputBegan:Connect(function(key, gameProcessedEvent)
    ...
    if key.KeyCode == Skibidi.MenuKeybind and not gameProcessedEvent then
        toggle = not toggle
...
local settings = {
 ...
 local function createEsp(player)
    ...
    drawings.boxinline.Thickness = 2;
...
local function updateEsp(player, esp)
    ...
    if character and Skibidi.BoxEsp and character:FindFirstChild("HumanoidRootPart") then
        local cframe = character:GetModelCFrame()
        local position, visible, depth = wtvp(character.HumanoidRootPart.Position)
        esp.box.Visible = visible
        esp.boxoutline.Visible = visible
        esp.boxinline.Visible = visible

        if cframe and visible then
            local scaleFactor = 1 / (depth * tan(rad(camera.FieldOfView / 2)) * 2) * 1000
            local width, height = round(4 * scaleFactor, 7 * scaleFactor)
            ...
            -- Adjust esp.boxinline size and position properly
            esp.boxinline.Size = esp.box.Size - newVector2(2, 2) -- Make sure to use the correct newVector2 function
            esp.boxinline.Position = esp.box.Position + newVector2(1, 1) -- Adjust to maintain the inline position correctly
        end
    else
 ...
 for i = 1, 3 do
    task.wait(0.5)
    Menu.Notify("erm what the sigma | Loaded.", 5)
end
